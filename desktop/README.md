...
租户管理员可在“成员管理”或“组织管理”中使用“批量导入”：先下载对应 `.xlsx` 模板，再上传文件、
查看服务端预览与逐行错误，最后确认批次。用户导入和组织导入共用四步抽屉流程，成功后会自动刷新当前
页面；错误报告下载不会暴露其他租户的批次数据。

用户导入模板提供中文填写页、字段说明和数据字典；填写页保留隐藏系统映射行，并内置冻结窗格、
自动筛选以及角色编码、成员状态下拉选项。请在中文表头下覆盖或删除示例数据后上传。

上传页会独立展示已选文件、处理状态和持久错误信息；“校验并进入预览”成功后自动推进到数据预览。
万级用户校验需要计算 bcrypt 密码摘要，界面会保持加载提示，请勿在处理完成前关闭导入抽屉。
导入完成后的租户成员页使用服务端分页，默认每页展示 50 名成员；翻页只请求当前页用户和角色，
不会把一万名成员一次性加载到前端。
确认导入后接口会立即进入结果步骤并显示排队/执行进度；抽屉关闭或页面刷新时会恢复当前待处理批次，
终态成功后再刷新成员或组织列表。进度查询使用轻量状态接口，不重复下载完整逐行预览。
