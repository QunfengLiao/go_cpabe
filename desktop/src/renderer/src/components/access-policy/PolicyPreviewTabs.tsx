...
import type { PolicyTreeNode, ValidationError } from "./tree/types";
  ...
  return (
    <section className="policy-preview-tabs">
      <div className="preview-tab-list" role="tablist" aria-label="策略预览">
        ...
        <button type="button" className={activeTab === "validation" ? "active" : ""} onClick={() => setActiveTab("validation")}>校验结果 {errors.length > 0 ? `(${errors.length})` : ""}</button>
      </div>
      <div className="preview-tab-panel">
        {activeTab === "expression" && <PolicyExpressionPreview expression={expression} errors={errors} />}
