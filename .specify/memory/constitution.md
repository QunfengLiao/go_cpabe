...
常量、字段映射、路由注册和显而易见的胶水代码 MAY 少写解释性块注释，但不得以此规避
关键逻辑说明。

Go 后端代码的注释必须满足以下硬性约束：

- 每一个函数和方法前 MUST 有注释，包括导出函数、导出方法、私有函数、私有方法和测试辅助函数。
- 导出的函数、方法、类型、接口、常量和变量 MUST 符合 GoDoc 规范，注释必须以对应标识符
  名称开头，例如 `// Login 处理用户登录请求。`、`// UserService 负责用户相关业务逻辑。`。
- 私有函数和私有方法的注释 MUST 说明职责、调用场景、关键输入输出含义、边界条件以及可能的
  副作用，例如写数据库、写 Redis、修改 session、生成 Token 或保存文件。
- 数据库实体、DAO、Model 和 Domain 字段 MUST 说明表或结构体的业务含义；关键字段 MUST 说明
  字段来源、是否可为空、是否属于敏感信息、是否参与权限判断或外部响应。
- Handler 注释 MUST 说明该接口处理的业务动作、请求来源、鉴权前提、主要副作用和错误响应边界。
- Service 注释 MUST 说明业务规则、事务边界、幂等性、权限前置条件和跨仓储协作原因。
- Repository 注释 MUST 说明查询或写入的数据范围、唯一性假设、未找到记录时的返回语义和数据库
  副作用。
- Middleware 注释 MUST 说明拦截哪些请求、从哪里读取登录态或租户上下文、校验成功后向
  `gin.Context` 写入哪些键和值，以及校验失败时如何中止请求。
- 涉及复杂 if 分支、事务、缓存、并发、权限校验、session/JWT、外部 API 调用、文件上传或
  密码学边界的代码，MUST 使用必要块级注释解释设计取舍和风险控制。
...
每个会新增或修改 Go 业务代码的 plan MUST 在宪章检查中说明注释策略；每个对应 tasks
文档 MUST 包含“关键注释和可读性检查”任务。实现交付说明 MUST 明确是否完成以下检查：
所有函数/方法已有前置注释、导出标识符符合 GoDoc、核心模块注释解释了业务语义和安全边界、
没有无意义逐行注释。如未完成，MUST 说明原因和后续补齐路径。
...
- 每个 SpecKit 生成文档 MUST 遵守“语言与文档规范”，模板标题和说明生成时 MUST 使用
  简体中文。
- 每个会新增或修改 Go 业务代码的 plan MUST 写明注释策略，每个对应 tasks 文档 MUST
  包含“关键注释和可读性检查”任务。
- 每次 AI 生成或修改 Go 业务代码后，MUST 检查所有函数/方法是否有前置注释，导出的函数、
  方法、类型、接口、常量和变量是否符合 GoDoc 标识符前缀规范，数据库实体字段、Handler、
  Service、Repository、Middleware 是否解释了业务语义、副作用和关键边界。
- 涉及安全、认证、权限、Token、密码、文件上传、加密算法或访问控制的实现 MUST 说明安全边界。
  交付说明 MUST 明确该检查是否完成。
- 实现前 MUST 完成 spec、plan、tasks；实现后 MUST 记录验证结果和任何偏离宪章的风险。

**版本**: 2.3.0 | **批准日期**: 2026-07-05 | **最后修订**: 2026-07-08
